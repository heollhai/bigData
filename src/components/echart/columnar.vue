  ...
  methods: {
    ...
    initChartData() {
      ...
      switch (this.option.axisLabel) {
        case "twoLine":
          // x 轴分两行显示
          (this.echart3D.xAxis.axisLabel.interval = 0),
            (this.echart3D.xAxis.axisLabel.formatter = function(value, index) {
              if (index % 2 != 0) {
                return "\n\n" + value;
              } else {
                return value;
        ...
        case "incline":
          // x轴倾斜显示
          this.echart3D.xAxis.axisLabel.rotate = 40;
          break;
      }
      // x轴数据
      this.echart3D.xAxis.data = this.option.xData;
      //  xy轴 标题 颜色
      this.echart3D.yAxis.axisLabel.color = this.option.XYColor;
      this.echart3D.xAxis.axisLabel.color = this.option.XYColor;
      // 标题   左右侧颜色
    ...
    judge(item, option, num) {
      if (this.option.series[num] !== undefined) {
        ...
        item.itemStyle.color = option.series[num].color;
